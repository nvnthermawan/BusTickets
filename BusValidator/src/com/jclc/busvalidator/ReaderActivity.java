...
public class ReaderActivity extends Activity {
	    ...
	    if (requestCode == 0) {
	        if (resultCode == RESULT_OK) {
	            ...
	            if(contents != null && contents.length()>=0 && contents.contains(",")){
		            ...
		            id_user = Integer.parseInt(contents.split(",")[1]);
	            }
	            else {
	            	id_ticket = -1;
	            	id_user = -1;
	            ...
	            ValidateTicket vt = new ValidateTicket();
	    		vt.execute();

	            //Toast.makeText(getApplicationContext(), "Empty QR Code!", Toast.LENGTH_LONG).show();
	...
	public class ValidateTicket extends AsyncTask<Object, Object, Object>{
		...
		public void postData() {
		    ...
		    HttpPost httppost = new HttpPost("http://192.168.102.50:1122/validateTicket");
		    ...
		    try {
				...
				try {
					...
					if(theresponse.equals("OK")){
						...
						if(holder2.has("date")){
							response2 += "\n Valid till " + holder2.getString("date");
						}
					}
					else {
